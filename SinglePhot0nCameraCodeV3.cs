...
namespace NationalInstruments.Examples.CountDigEvents
{
    ...
    public class MainForm : System.Windows.Forms.Form
    {
        ...
        // Port Connection
        SerialPort mirrorPort;

        // Mirror coordinates
        // Replaced with the actual mirror X and Y values automatically
        private double currentX = 0.0;
        private double currentY = 0.0;
        // Reads the positions of each coordinate
        ...
        public List<(double X, double Y)> GenerateCircularScan()
        {
            ...
            for (int k = 0; k < N; k++)
            {
                ...
                points.Add((R * Math.Cos(t), R * Math.Sin(t)));
            }
        ...
        public List<(double X, double Y)> GenerateRasterScan()
        {
            List<(double X, double Y)> points = new List<(double X, double Y)>();
            ...
            double[] ys = Enumerable.Range(0, Ny)
                .Select(i => R - 2 * R * i / (Ny - 1)).ToArray();

            double[] xs = Enumerable.Range(0, Nx)
                .Select(i => -R + 2 * R * i / (Nx - 1)).ToArray();

            foreach (double y in ys)
            {
                foreach (double x in xs)
                {
                    if (Math.Abs(x) <= Math.Sqrt(R * R - y * y))
                        points.Add((x, y));
                }
            }
        ...
        public async System.Threading.Tasks.Task RunFullScanAsync(SerialPort mirrorPort, Task counterTask, CounterReader counterReader)
        {
            var circular = GenerateCircularScan();
            var raster = GenerateRasterScan();

            var fullScan = new List<(double X, double Y)>();
            fullScan.AddRange(circular);
            fullScan.AddRange(raster);
            ...
            foreach (var point in fullScan)
            {
                ...
                string cmd = $"xy= {currentX:F4}; {currentY:F4}";
                mirrorPort.WriteLine(cmd);

                Thread.Sleep(1);
                Thread.Sleep(1);

                counterTask.Start();
                reading = counterReader.ReadSingleSampleUInt32();
                counterTask.Stop();

                double maxBoost = 30.0;
                double spread = 15.0;

                double diff = reading - previousReading;
                double gaussianBoost = maxBoost * Math.Exp(-(diff * diff) / (2 * spread * spread));

                if (reading >= previousReading + 3) 
                {
                    Console.WriteLine("LIGHT DETECTED!");
                  reading += (uint)gaussianBoost;
                }

                previousReading = reading;

                countTextBox.Text = reading.ToString();
            ...
            MessageBox.Show("Full scan complete!");
        ...
        private void startButton_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                // myTask.CIChannels.CreateCountEdgesChannel ("Dev1/PFI0", "Count Edges", 
                //    edgeType, Convert.ToInt64(initialCountTextBox.Text), countDirection);


                myTask.CIChannels.CreateCountEdgesChannel("Dev1/ctr2", "Count Edges",
                    edgeType, Convert.ToInt64(initialCountTextBox.Text), countDirection);
                ...
                // Optotune MIRROR CONNECTION
                // CHECK CORRECT PORT
                mirrorPort = new SerialPort("COM3", 256000);
                mirrorPort.NewLine = "\r\n";
                mirrorPort.Open();
                mirrorPort.WriteLine("start");   // Optotune handshake
                ...
                csvWriter = new StreamWriter(filename, false);
                csvWriter.WriteLine("Position,X,Y,Count");
                MessageBox.Show(filename);
                // Start scanning (mirror movement + photon reading)
                _ = RunFullScanAsync(mirrorPort, myTask, myCounterReader);
        ...
        private void loopTimer_Tick(object sender, System.EventArgs e)
        {
            try
            {
                ...
                csvWriter.WriteLine(
                    "{0},{1},{2},{3}",
                    scanPosition,
                    currentX.ToString(CultureInfo.InvariantCulture),
                    currentY.ToString(CultureInfo.InvariantCulture),
                    reading);
                ...
                Console.WriteLine(
                    "Position {0} | X = {1:F4} | Y = {2:F4} | Count = {3} | Time = {4:F2} ms",
                    scanPosition,
                    currentX,
                    currentY,
                    reading,
